...
- 记录任意 M 点到任意 S 点的最小步数，无法到达的单元格填充-1 -> dist[m_i][j]
  设存在 nb 个 M 点则：
  dist[m_i][nb] -> 记录该 M 点到起点最小步数
  dist[m_i][nb+1] -> 记录该 M 点到 T 点最小步数

dist 二维数组，dist[m_i][j]:

- 从 M 点 m_i 出发到最近 S 点的最小的步数 dist[m_i][i]
- 从 M 点 m_i 出发到起点的最小步数 dist[m_i][nb]
- 从 M 点 m_i 出发到起点的最小步数 dist[m_i][nb+1]
